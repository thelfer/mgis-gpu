...
namespace mgis::gpu {
  ...
  bool cuda_execute(const KernelType kernel,
    ...
    if constexpr (!IsTimed) {
      ...
      auto success = kernel(K_values, sig_values, eto_values, n);
      deallocate(K_values);
    ...
    cudaMemcpy(eto_values.data(), eto_host.data(), 6 * n * sizeof(real), cudaMemcpyHostToDevice);
    kernel(K_values, sig_values, eto_values, n);
    std::vector<real> K_warmup(6 * 6 * n);
    std::vector<real> sig_warmup(6 * n);
    cudaMemcpy(K_warmup.data(), K_values.data(), 6 * 6 * n * sizeof(real), cudaMemcpyDeviceToHost);
    ...
    // D2H
    std::vector<real> K_host(6 * 6 * n);
    std::vector<real> sig_host(6 * n);
    d2h_timer.begin();
    cudaMemcpy(K_host.data(), K_values.data(), 6 * 6 * n * sizeof(real), cudaMemcpyDeviceToHost);
...
int main() {
  ...
  success = mgis::gpu::execute<mgis::gpu::is_timed, use_gpu_timing>(
                mgis::gpu::stlpar_kernel, n, "elasticity2", stlpar_name, num_threads) &&
            success;
#endif /* MGIS_HAS_STL_PARALLEL_ALGORITHMS */
#ifdef MGIS_GPU_HAS_CUDA_SUPPORT
  success = mgis::gpu::cuda_execute<mgis::gpu::is_timed>(
                mgis::gpu::cuda_kernel, n, "elasticity2", "cuda") &&
            success;
#endif /* MGIS_GPU_HAS_CUDA_SUPPORT */
